...
// Запуск:     echo_server_windows.exe <port> <protocol> <ip_version>
        ...
        while (1) {
            ...
            if (client_addr.ss_family == AF_INET6) {
                ...
                printf("[Client] IPv6: [%s]:%d scope_id=%u\n", 
                       client_ip, ntohs(addr6->sin6_port), addr6->sin6_scope_id);
            } else {
        ...
        while (1) {
            ...
            if (client_addr.ss_family == AF_INET6) {
                ...
                printf("[Client] IPv6: [%s]:%d scope_id=%u >> %s\n",
                       client_ip, ntohs(addr6->sin6_port), addr6->sin6_scope_id, buffer);
            } else {
                struct sockaddr_in* addr4 = (struct sockaddr_in*)&client_addr;
                inet_ntop(AF_INET, &addr4->sin_addr, client_ip, sizeof(client_ip));

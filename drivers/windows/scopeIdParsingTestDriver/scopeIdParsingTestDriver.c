...
{
    RtlStringCbPrintfA(buffer, buffer_size, "%02X%02X:%02X%02X:%02X%02X:%02X%02X:%02X%02X:%02X%02X:%02X%02X:%02X%02X",
                       ipv6[0], ipv6[1], ipv6[2], ipv6[3], ipv6[4], ipv6[5], ipv6[6], ipv6[7], ipv6[8], ipv6[9],
                       ipv6[10], ipv6[11], ipv6[12], ipv6[13], ipv6[14], ipv6[15]);
}
...
NTSTATUS DriverEntry(PDRIVER_OBJECT DriverObject, PUNICODE_STRING RegistryPath)
{
    ...
    // Тест IPv6 с scope_id (имя интерфейса которого нету в системе, соотвесвенно scope_id не запишется и останется
    // старым)
    ...
    */
    ...
    err = net_address_parse("fe80::a921:2d1f:61fb:8942%Ethernet0", NET_AF_INET6, &addr);
    DbgPrint("  Parse result: %d (expected -3)\n", err);
    DbgPrint("  scope_id: %u (expected 9)\n", addr.scope_id);

    DbgPrint("\n[Test 9] Invalid IPV6 format (without [...] and without port)\n");
    err = net_address_parse("fe80::a921:2d1f:61fb:8942%13", NET_AF_INET6, &addr);
    DbgPrint("  Parse result: %d (expected -3)\n", err);
    DbgPrint("  scope_id: %u (expected 13)\n", addr.scope_id);

    DbgPrint("\n[Test 10] Invalid IPV6 format (without [...] but with port: ...%...::9090)\n");
    err = net_address_parse("fe80::a921:2d1f:61fb:8942%Ethernet0::9090", NET_AF_INET6, &addr);
    DbgPrint("  Parse result: %d (expected -8)\n", err);
    ...
    // fe80::1
    uint8_t ipv6[] = {0xFE, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01};

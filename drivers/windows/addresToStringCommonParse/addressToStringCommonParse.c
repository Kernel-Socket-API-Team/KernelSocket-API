...
NTSTATUS DriverEntry(PDRIVER_OBJECT DriverObject, PUNICODE_STRING RegistryPath) {
  ...
  char *buffer =
      (char *)ExAllocatePool2(POOL_FLAG_NON_PAGED, bufferSize, 'bufC');
  ...
  char *bufferShort =
      (char *)ExAllocatePool2(POOL_FLAG_NON_PAGED, bufferSize, 'bufS');
  ...
  for (int i = 0; i < ARRAYSIZE(testPorts); i++) {
    ...
    DbgPrint(
        "Port test: %u | addr string='%s' | conv error=%d\n", port,
             portBuffer, error);

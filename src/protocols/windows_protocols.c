...
net_error_t windows_net_socket_create(net_family_t family, net_protocol_t protocol, net_socket_type_t type,
                                      net_socket_t** socketOut)
{
    ...
    status = g_WskContext.ProviderNpi.Dispatch->WskSocket(g_WskContext.ProviderNpi.Client, // Client
...
net_error_t windows_net_socket_bind(net_socket_t* sock, const net_address_t* addr)
{
    ...
    case NET_SOCK_TYPE_TCP_LISTEN:
        status =
            ((PWSK_PROVIDER_LISTEN_DISPATCH)impl->wsk_socket->Dispatch)->WskBind(impl->wsk_socket, pSockAddr, 0, irp);
        break;
    ...
    case NET_SOCK_TYPE_UDP:
        status =
            ((PWSK_PROVIDER_DATAGRAM_DISPATCH)impl->wsk_socket->Dispatch)->WskBind(impl->wsk_socket, pSockAddr, 0, irp);
        break;
...
net_error_t windows_net_socket_accept(net_socket_t* server, net_socket_t** client_out)
{
    ...
    status = ((PWSK_PROVIDER_LISTEN_DISPATCH)impl->wsk_socket->Dispatch)
                 ->WskAccept(impl->wsk_socket, 0, NULL, NULL, NULL, NULL, irp);
    ...
    PWINDOWS_SOCKET_IMPL client_impl =
        (PWINDOWS_SOCKET_IMPL)ExAllocatePool2(POOL_FLAG_NON_PAGED, sizeof(WINDOWS_SOCKET_IMPL), 'cskW');
                                       ...
                                       size_t* received)
{
    ...
    else
    {
        ...
        status = ((PWSK_PROVIDER_DATAGRAM_DISPATCH)impl->wsk_socket->Dispatch)
                     ->WskReceiveFrom(impl->wsk_socket, &wsk_buf, 0, (PSOCKADDR)&addr, &addr_len, NULL, NULL, irp);

...
Функция ожидает завершения инициализации библиотеки. Данная функция **МОЖЕТ блокировать поток**. Рекомендуется вызывать **в отдельном рабочем потоке**, чтобы не блокировать основной поток выполнения. При этом поведение зависит от параметра `limitMS`:

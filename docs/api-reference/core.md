...
Функция запускает процесс инициализации библиотеки. Функция **НЕ блокирует вызывающий поток**. В случае повторного вызова, новая регистрация производится только в случае если прошлый вызов вернул ошибку. При этом не гарантирует, что библиотека готова к использованию сразу после возврата.
...
Функция ожидает завершения инициализации библиотеки. Данная функция **МОЖЕТ блокировать поток**. Рекомендуется вызывать **в отдельном рабочем потоке**, чтобы не блокировать основной поток выполнения. При этом поведение зависит от параметра `limitMS`. В случае успешного выполнения библиотека гарантированно готова к работе.

### Параметры

* **[in]** `limitMS` - время ожидания активации библиотеки:
  * `NET_WAIT_INFINITE` — ожидание без ограничения по времени (аналог "бесконечного ожидания")
...
Проверяет, завершена ли инициализация библиотеки.Функция **НЕ блокирует поток**. Выполняет мгновенную проверку состояния.
...
Освобождает ресурсы, выделенные библиотекой. При этом, важно вызывать функцию даже если не производилась активация (например, в случае ошибки), но регистрация была успешно выполнена. Должна вызываться при завершении работы.
Закрывает внутренние дескрипторы, освобождает память и завершает фоновые задачи. После вызова библиотека считается неинициализированной.
...
Библиотека спроектирована таким образом, что функция `net_activate()` должна быть вызвана в каждом рабочем потоке, который планирует использовать сетевые операции. Это связано с тем, что активация библиотеки в одном потоке не гарантирует её доступность в других потоках.

Очистка ресурсов, в отличие от активации, происходит централизованно — достаточно один раз вызвать `net_cleanup()` после завершения всех рабочих потоков.

Пример для Windows (аналогично для Linux):

```c
// Регистрация (один раз при загрузке)
net_register();

// В каждом рабочем потоке — своя активация

...
* При указании порта (например, `[::1]:8080`) квадратные скобки обязательны для IPv6 адресов иначе функция вернет ошибку тип `NET_ERROR_INVALID_PARAM`
...
* Для IPv4 адресов скобки не используются и не требуются

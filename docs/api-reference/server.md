...
### Описание
Принимает входящее TCP-соединение. **Блокирует выполнение потока** до установки нового соединения. Создаёт и возвращает новый сокет для взаимодействия с клиентом.

Важные особенности:

* Создаваемый сокет имеет тип `NET_SOCK_TYPE_TCP_CONNECTION`

* Удалённый адрес клиента автоматически сохраняется в поле `remote_addr`

* Получить адрес клиента можно через `net_socket_get_remote_address()` или далее через вызов функции `net_socket_receive()`

**Важно!** После `net_socket_accept` клиентский сокет сразу готов к отправке данных без дополнительных вызовов `net_socket_connect`. Более того, лишний вызов `net_socket_connect` для данного сокета приведёт к конфликту, после чего он станет непригодным для отправки сообщений.
...
### Модификация TCP сервера для использования IPV6
